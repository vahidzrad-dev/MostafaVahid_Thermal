...
v_R = Expression(("t <= tc ? v_f*t/tc : v_f", "0"), t=0, tc=t_cut, v_f=v_f, degree=0)   # velocity at part of left edge.

# v_lb = interpolate(v_R, V_u)
# v0.assign(update_v(u_, u0, v0, a0))

# load_top = Expression(("0.0", "t"), t=0.0, degree=1)
# bc_v_edge = DirichletBC(V_u, v_R, v_D)
# bc_v = [bc_v_edge]
...
E_ui = rho * dot(a0, u_t) * dx + inner(sigma(u_, T_), epsilon_e(u_t, T_)) * dx - dot(traction, u_t) * ds
# E_d = 1.0/(4.0 * cw) * Gc * (d_**2.0/ell * dx + ell * inner(grad(d_), grad(d_)) * dx)
E_di = 1.0/(4.0 * cw) * Gc * (d_**2.0/ell * dx + ell * inner(grad(d_), grad(d_)) * dx)
# E_T = rho * c / deltaT * (T_ - T0) * T_t * dx - k * inner(grad(T_), grad(T_t)) * dx
E_Ti = rho * c * (T_ - T0)/deltaT * T_t * dx + k * inner(nabla_grad(T_), grad(T_t)) * dx - \
       gamma * T_t * dx + J_bar * T_t * ds(2)
