...
# Define mesh
mesh = BoxMesh(Point(0., 0., 0.), Point(50., 50., 5.), 50, 50, 5)
...
def right(x, on_boundary):
    return near(x[0], 50.) and on_boundary

def rear(x, on_boundary):
    return near(x[2], 0.) and on_boundary

def front(x, on_boundary):
    return near(x[2], 5.) and on_boundary
...
bc_rear = DirichletBC(V, zero, rear)
bc_front = DirichletBC(V, zero, front)
